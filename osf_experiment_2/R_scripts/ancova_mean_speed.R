# Mean Speed with Steering Acceleration Covariate ----------------------------------------------------------------
...
main="Mean Speed for SS, SL and Control Groups"

# Long format
mean_speed_df <- main_df %>%
  select(subject_id, condition,total_steering_acceleration_1,low_vis_mean_speed_10, high_vis_mean_speed_10) %>%
...
mean_speed_ancova_table <- get_anova_table(mean_speed.aov)

# Simple main effect analysis

mean_speed_df %>%
  group_by(visibility) %>%
  anova_test(mean_speed ~ total_steering_acceleration_1 + condition,effect.size = "pes")

pwc <- mean_speed_df %>% 
  group_by(visibility) %>%
  emmeans_test(
    mean_speed ~ condition, covariate = total_steering_acceleration_1, p.adjust.method = "bonferroni"
  )
...
pwc.filtered <- pwc %>% filter(visibility == "high_vis_mean_speed_10")
lp + 
  stat_pvalue_manual(
  ...
  labs(
    subtitle = get_test_label(mean_speed.aov,  detailed = TRUE),
...
# Plot using ggplot
mean_speed_plot_ancova <- ggplot(adj_means_df, aes(x = visibility, y = mean_speed_devs, color = familiarity, group = familiarity)) +
